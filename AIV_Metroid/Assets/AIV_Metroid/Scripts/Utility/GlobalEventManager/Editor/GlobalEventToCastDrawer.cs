...
public class GlobalEventToCastDrawer : PropertyDrawer
{
    ...
    private const string templatesRoot = "Assets/AIV_Metroid/Database/GlobalEventArgs/";
    ...
    public override VisualElement CreatePropertyGUI(SerializedProperty property) {
        ...
        var eventToCastField = new PropertyField(property.FindPropertyRelative("eventToCast"));
        var eventArgsField = new PropertyField(property.FindPropertyRelative("message"));
        ...
        eventToCastField.RegisterValueChangeCallback((evt) => {
            UpdateEventArgsProperty(
                property.FindPropertyRelative("message").FindPropertyRelative("args"),
    ...
    private void UpdateEventArgsProperty (SerializedProperty property, 
        GlobalEventIndex eventType) {
        EventArgsTemplate template = templatesAssets[(int)eventType];
        property.arraySize = template.VariablesName.Length;
        for (int i = 0; i < property.arraySize; i++) {
            UpdateArg(property.GetArrayElementAtIndex(i),
                template.VariablesName[i], template.VariablesType[i]);
        }
        property.serializedObject.ApplyModifiedProperties();
    }

    private void UpdateArg (SerializedProperty property, string name, ExtendedVariableType variableType) {
        property.FindPropertyRelative("variableName").stringValue = name;
        property.FindPropertyRelative("type").enumValueIndex = (int)variableType;
    }
